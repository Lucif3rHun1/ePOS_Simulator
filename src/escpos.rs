...
///
/// REVERTED 2026-07-26 from `GS ( L` (fn112 store + fn50 print) back to
/// this. `GS ( L` is Epson's documented "modern" replacement and is what
/// python-escpos etc. use, but on real hardware (Epson TM-T82X) a ~32KB
/// image sent that way came out as corrupted dashes across the receipt —
/// most likely fn112's "store the whole image in the print buffer first"
/// step overflowing that printer's buffer for a large single-shot image,
/// something `GS v 0`'s more streaming-oriented raster print doesn't hit
/// the same way. This single-command `GS v 0` form (below) was verified
/// working correctly on the same real printer (just slower) before the
/// `GS ( L` switch — see git history (commit 9bbabb0) if `GS ( L` is
/// ever revisited, e.g. behind a size threshold or a printer capability
...
/// into per-row calls (as this used to do, even earlier) turns one
/// continuous raster print into hundreds of separate mechanical
/// stop/start cycles — very slow, and any per-call feed overshoot
/// accumulates into large extra vertical gaps. yL/yH supports up to
/// 65535 rows, comfortably covering any receipt-length image in one call.
...
mod tests {
    ...
    fn raster_single_row_paper_width_8() {
        // 1 row of 8 black pixels at 8-dot paper width → GS v 0, m=0,
        // bytes-per-row=1, yL=1, then the 1-byte raster.
        let img = vec![0xFF];
        let out = raster_print(&img, 8, 1, 8);
        assert_eq!(out, vec![0x1D, b'v', b'0', 0, 1, 0, 1, 0, 0xFF]);
    }

    #[test]
    fn raster_multi_row_single_command() {
        // 2 rows on a 16-dot paper, each row is 2 bytes of 0xFF. Must be
        // ONE GS v 0 command covering all rows (yL/height=2), not
        // per-row commands — calling it once per row turns a continuous
        // image print into hundreds of separate print-engine feed
        // cycles, which was both very slow and stacked up extra vertical
        // gaps between rows.
